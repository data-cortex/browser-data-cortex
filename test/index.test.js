...
describe('DataCortex Library Tests', () => {
  ...
  describe('Initialization', () => {
    ...
    test('should add error handler when requested', () => {
      ...
      assert.strictEqual(addEventListenerMock.mock.calls[0].arguments[0], 'error');
  ...
  describe('Local Storage Integration', () => {
    ...
    test('should persist device tag in localStorage', () => {
      ...
      const deviceTag = JSON.parse(global.localStorage.getItem('dc.device_tag'));
  ...
  describe('Error Handling', () => {
    test('should handle JavaScript errors when error handler is enabled', () => {
      ...
      global.window.dispatchEvent(new global.window.ErrorEvent('error', errorEvent));
      
      // Check that log was created
      const logList = JSON.parse(global.localStorage.getItem('dc.log_list') || '[]');
      const errorLog = logList.find(log => log.log_line.includes('Javascript Error'));
      assert(errorLog);
